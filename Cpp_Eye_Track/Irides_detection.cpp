...
void detectEyesRight(cv::Mat &frame, cv::CascadeClassifier &faceCascade, cv::CascadeClassifier &eyeCascade)
{
  cv::Mat grayscale;
  ...
  cv::Rect eyeRect = getRightmostEye(eyes);
  cv::Mat eye = face(eyeRect); // crop the rightmost eye
...
void detectEyesLeft(cv::Mat &frame, cv::CascadeClassifier &faceCascade, cv::CascadeClassifier &eyeCascade)
{
  cv::Mat grayscale;
  cv::cvtColor(frame, grayscale, CV_BGR2GRAY); // convert image to grayscale
  cv::equalizeHist(grayscale, grayscale); // enhance image contrast 
...
int main()
{
  ...
  cv::VideoCapture cap(0); // the first webcam connected to your PC
  if (!cap.isOpened())
  {
  ...
  while (1)
  {
      ...
      if (!frame.data) break;
      detectEyesRight(frame, faceCascade, eyeCascade);
      detectEyesLeft(frame, faceCascade, eyeCascade);
